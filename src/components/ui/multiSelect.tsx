...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "flex flex-wrap gap-1 p-2 py-2 border border-muted rounded-lg bg-background h-14",
        className
      )}
      {...props}
    >
      {value.map((item, index) => (
        <Badge
          key={item}
          className={cn(
            "px-4 rounded-lg flex items-center gap-1 bg-white border border-MainBlue text-gray-700",
        ...
        >
          ...
          >
            ...
            <RemoveIcon className="h-4 w-4 hover:stroke-destructive text-MainBlue" />
          </button>
        </Badge>
      ))}
      {children}
